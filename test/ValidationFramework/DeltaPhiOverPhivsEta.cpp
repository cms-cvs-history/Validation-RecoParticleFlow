...
DeltaPhiOverPhivsEta::DeltaPhiOverPhivsEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaPhiOverPhivsEta::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaPhi/Phi)/Eta");
...
void DeltaPhiOverPhivsEta::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaPhiOverPhi_vs_Eta.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaPhiOverPhivsEta/"<< name << "Compare_etBin";
