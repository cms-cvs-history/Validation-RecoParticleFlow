...
DeltaEtOverEtvsEt::DeltaEtOverEtvsEt(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtOverEtvsEt::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt/Et)/Et");
...
void DeltaEtOverEtvsEt::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEtOverEt_vs_Et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtOverEtvsEt/"<< name << "Compare_etBin";
