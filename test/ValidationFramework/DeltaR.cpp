...
DeltaR::DeltaR(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaR::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigmaDeltaR");
...
void DeltaR::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaR.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaR/"<< name << "Compare_etBin";
