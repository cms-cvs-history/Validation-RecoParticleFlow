...
DeltaEtaOverEtavsEt::DeltaEtaOverEtavsEt(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtaOverEtavsEt::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEta/Eta)/Et");
...
void DeltaEtaOverEtavsEt::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEtaOverEta_vs_et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtaOverEtavsEt/"<< name << "Compare_etBin";
