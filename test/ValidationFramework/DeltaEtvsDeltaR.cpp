...
DeltaEtvsDeltaR::DeltaEtvsDeltaR(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtvsDeltaR::histo_conf(int type)
{
	if(type==1)
	{
		href->GetXaxis()->SetTitle("#Delta R");
		href->GetYaxis()->SetTitle("Events");
		href->GetYaxis()->SetTitleOffset(1.5);
		
		hnew->GetXaxis()->SetTitle("#Delta R");      
		//	href->GetYaxis()->SetTitle("Events");
		
		h1dResults_passed->GetXaxis()->SetTitle("#Delta R");
		...
		hrefSigmaETOverETvsETsigma->GetXaxis()->SetTitle("#Delta R");
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt)/#Delta R");
...
void DeltaEtvsDeltaR::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEt_vs_DeltaR.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtvsDeltaR/"<< name << "Compare_etBin";
