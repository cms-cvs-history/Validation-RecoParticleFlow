...
DeltaEtaOverEtavsEta::DeltaEtaOverEtavsEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtaOverEtavsEta::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEta/Eta)/Eta");
...
void DeltaEtaOverEtavsEta::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEtaOverEta_vs_Eta.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtaOverEtavsEta/"<< name << "Compare_etBin";
