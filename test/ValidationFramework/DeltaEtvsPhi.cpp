...
DeltaEtvsPhi::DeltaEtvsPhi(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtvsPhi::histo_conf(int type)
{
	if(type==1)
	{
...
void DeltaEtvsPhi::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEt_vs_Phi.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtvsPhi/"<< name << "Compare_etBin";
