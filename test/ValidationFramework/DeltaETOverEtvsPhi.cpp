...
DeltaETOverEtvsPhi::DeltaETOverEtvsPhi(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaETOverEtvsPhi::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt/Et)/Phi");
...
void DeltaETOverEtvsPhi::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEtOverEt_vs_Phi.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtOverEtvsPhi/"<< name << "Compare_etBin";
