...
vsBase::vsBase(PlotCompareUtility *pc)
{
	this->pc=pc;
...
int vsBase::fetch_histos(int histo_index)
{
	...
	hnew2d = (TH2F *)pc->GetNewHisto(name);
	href2d = (TH2F *)pc->GetRefHisto(name);
...
void vsBase::histo_conf(int type)
{
	if(type==1)
	{
...
void vsBase::loop_over_et_bins(int histo_index, int bin_index)
{
	// loop over Et bins
	int index=histo_index;
	int bin=bin_index;
	...
	hnew = hnew2d->ProjectionY(hname_new.str().c_str(),bin,bin);
	href = href2d->ProjectionY(hname_ref.str().c_str(),bin,bin); 
...
void vsBase::draw_score_vs_ET(int histo_index)
{
	int index=histo_index;
...
void vsBase::draw_test_plot_and_delta_et()
{
	...
	hname_new << "tmp_hnew_" ;
	hname_ref << "tmp_href_" ;
	hnew = hnew2d->ProjectionY(hname_new.str().c_str());
	href = href2d->ProjectionY(hname_ref.str().c_str()); 
...
void vsBase::draw_test_plot_sigma_ET_vs_ET()
{
...
void vsBase::report(int histo_index)
{
	int index=histo_index;
...
void vsBase::print_gifs(int type,int bin)
{
