...
DeltaEta::DeltaEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEta::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigmaEta");
...
void DeltaEta::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEta.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEta/"<< name << "Compare_etBin";
