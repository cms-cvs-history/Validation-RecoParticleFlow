...
DeltaEtvsEta::DeltaEtvsEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtvsEta::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt)/Eta");
...
void DeltaEtvsEta::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEt_vs_Eta.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtvsEta/"<< name << "Compare_etBin";
