...
DeltaEtavsEt::DeltaEtavsEt(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtavsEt::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEta)/Et");
...
void DeltaEtavsEt::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEta_vs_Et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtavsEt/"<< name << "Compare_etBin";
