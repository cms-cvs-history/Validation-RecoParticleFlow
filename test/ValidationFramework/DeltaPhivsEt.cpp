...
DeltaPhivsEt::DeltaPhivsEt(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaPhivsEt::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaPhi)/Et");
...
void DeltaPhivsEt::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaPhi_vs_Et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaPhivsEt/"<< name << "Compare_etBin";
