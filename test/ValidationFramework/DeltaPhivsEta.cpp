...
DeltaPhivsEta::DeltaPhivsEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaPhivsEta::histo_conf(int type)
{
	if(type==1)
	{
...
void DeltaPhivsEta::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaPhi_vs_Eta.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaPhivsEta/"<< name << "Compare_etBin";
