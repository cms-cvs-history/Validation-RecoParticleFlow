...
DeltaPhiOverPhivsEt::DeltaPhiOverPhivsEt(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaPhiOverPhivsEt::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaPhi/Phi)/Et");
...
void DeltaPhiOverPhivsEt::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaPhiOverPhi_vs_Et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaPhiOverPhivsEt/"<< name << "Compare_etBin";
