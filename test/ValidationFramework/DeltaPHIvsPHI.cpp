...
DeltaPHIvsPHI::DeltaPHIvsPHI(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaPHIvsPHI::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#Delta Phi)/Phi");
...
void DeltaPHIvsPHI::print_gifs(int type,int bin)
{
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaPhivsPhi/"<< name << "Compare_etBin";
