...
DeltaEtaOverEtavsPhi::DeltaEtaOverEtavsPhi(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtaOverEtavsPhi::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt/Eta)/Phi");
...
void DeltaEtaOverEtavsPhi::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEtaOverEta_vs_Phi.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtaOverEtavsPhi/"<< name << "Compare_etBin";
