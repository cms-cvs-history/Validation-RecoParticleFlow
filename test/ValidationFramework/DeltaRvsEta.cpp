...
DeltaRvsEta::DeltaRvsEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaRvsEta::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaR)/Eta");
...
void DeltaRvsEta::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaR_vs_Eta.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaRvsEta/"<< name << "Compare_etBin";
