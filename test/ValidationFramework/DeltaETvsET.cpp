...
DeltaETvsET::DeltaETvsET(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaETvsET::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt)/Et");
...
void DeltaETvsET::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaEt_vs_Et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtvsEt/"<< name << "Compare_etBin";
