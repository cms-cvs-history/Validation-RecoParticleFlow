...
DeltaRvsEt::DeltaRvsEt(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaRvsEt::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaR)/Et");
...
void DeltaRvsEt::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaR_vs_Et.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaRvsEt/"<< name << "Compare_etBin";
