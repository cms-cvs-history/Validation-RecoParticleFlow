...
DeltaEtOverEtvsDeltaR::DeltaEtOverEtvsDeltaR(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtOverEtvsDeltaR::histo_conf(int type)
{
	if(type==1)
	{
		href->GetXaxis()->SetTitle("#DeltaR");
		href->GetYaxis()->SetTitle("Events");
		href->GetYaxis()->SetTitleOffset(1.5);
		
		hnew->GetXaxis()->SetTitle("#DeltaR");      
		//	href->GetYaxis()->SetTitle("Events");
		
		h1dResults_passed->GetXaxis()->SetTitle("#DeltaR");
		...
		hrefSigmaETOverETvsETsigma->GetXaxis()->SetTitle("#DeltaR");
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt/Et)/#DeltaR");
		...
		hdumb->GetXaxis()->SetTitle("DeltaR");
...
void DeltaEtOverEtvsDeltaR::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		histo_name << name + "dR_Allbins";
		histo_name << ".gif";
		histo_c1->Print(histo_name.str().c_str(),"gif");
		
		string et_sigma_name = name + "SigmaDeltaEtOverEt_vs_DeltaR.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtOverEtvsDeltaR/"<< name << "Compare_etBin";
