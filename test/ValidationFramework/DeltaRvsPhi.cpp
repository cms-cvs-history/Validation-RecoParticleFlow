...
DeltaRvsPhi::DeltaRvsPhi(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaRvsPhi::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaR)/Phi");
...
void DeltaRvsPhi::print_gifs(int type,int bin)
{
	...
	if(type==0)
	{
		...
		string et_sigma_name = name + "SigmaDeltaR_vs_Phi.gif";
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaRvsPhi/"<< name << "Compare_etBin";
