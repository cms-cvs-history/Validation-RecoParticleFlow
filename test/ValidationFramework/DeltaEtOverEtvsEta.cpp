...
DeltaEtOverEtvsEta::DeltaEtOverEtvsEta(PlotCompareUtility *pc)
{
	this->pc=pc;
...
void DeltaEtOverEtvsEta::histo_conf(int type)
{
	if(type==1)
	{
		...
		hrefSigmaETOverETvsETsigma->GetYaxis()->SetTitle("#sigma(#DeltaEt/Et)/Eta");
...
void DeltaEtOverEtvsEta::print_gifs(int type,int bin)
{
	...
	else if(type==1)
	{
		...
		// print the result to gif
		stringstream histo_name;
		histo_name <<"DeltaEtOverEtvsEta/"<< name << "Compare_etBin";
